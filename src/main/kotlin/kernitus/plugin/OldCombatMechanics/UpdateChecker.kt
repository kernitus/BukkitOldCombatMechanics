...
class UpdateChecker(private val plugin: OCMMain) {
    ...
    private val autoDownload =
        Config.moduleSettingEnabled("update-checker", "auto-update") &&
                (VersionCompatUtils.versionIsNewerOrEqualTo(1, 18, 1) ||
                        Config.getConfig()
        ...
        if (updater.isUpdateAvailable) {
            messages.add(
                ChatColor.BLUE.toString() +
                        "An update for OldCombatMechanics to version " +
            ...
            if (!autoDownload) {
                ...
                try {
                    if (updater.downloadLatestVersion(
                            plugin.server.updateFolderFile,
                            plugin.file.name
                        )
                    )
                        messages.add(
                            ChatColor.GREEN.toString() +
                                    "Update downloaded. Restart or reload server to enable new version."
                        )
                    else throw RuntimeException()
                } catch (e: Exception) {
                    messages.add(
                        ChatColor.RED.toString() +
                                "Error occurred while downloading update! Check console for more details"
                    )
                    e.printStackTrace()
                }
            }
