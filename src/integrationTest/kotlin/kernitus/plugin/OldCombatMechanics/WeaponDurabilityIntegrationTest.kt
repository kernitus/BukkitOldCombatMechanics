...
class WeaponDurabilityIntegrationTest : FunSpec({
    ...
    fun attackNms(attacker: Player, target: LivingEntity) {
        runCatching {
            attacker.attack(target)
            return
        }
        val attackerHandle = attacker.javaClass.methods.firstOrNull { method ->
        ...
        runCatching {
            val managerField = attackerHandleClass.declaredFields.firstOrNull { field ->
            ...
            if (managerField != null) {
                ...
                val attackMethod = manager.javaClass.methods.firstOrNull { method ->
    ...
    fun resolveDebugFile(): File {
        ...
        return File("/home/ligma/gitstuff/BukkitOldCombatMechanics/build/weapon-durability-debug-$versionTag.txt")
    }
    ...
    fun describeNmsState(attacker: Player, victim: LivingEntity): String {
        return runCatching {
            val attackerHandle = attacker.javaClass.methods.firstOrNull { it.name == "getHandle" && it.parameterCount == 0 }
                ?.invoke(attacker) ?: return@runCatching "noAttackerHandle"
            val victimHandle = victim.javaClass.methods.firstOrNull { it.name == "getHandle" && it.parameterCount == 0 }
                ?.invoke(victim) ?: return@runCatching "noVictimHandle"

            fun flag(handle: Any, name: String): String? {
    ...
    fun setItemDamage(item: ItemStack, damage: Int) {
