...
class PlayerKnockbackIntegrationTest :
    FunSpec({
        ...
        fun putPending(
            ...
            val pendingClass =
                ModulePlayerKnockback::class.java.declaredClasses
                    .firstOrNull { it.simpleName == "PendingKnockback" }
                    ?: error("PendingKnockback inner class not found")
            val ctor =
                pendingClass.declaredConstructors.firstOrNull { constructor ->
        ...
        context("Knockback resistance") {
            test("modifiers are removed when resistance disabled") {
                withConfig {
                    ...
                    val modifier =
                        AttributeModifier(UUID.randomUUID(), "test", 0.5, AttributeModifier.Operation.ADD_NUMBER)
