...
class PacketCancellationIntegrationTest :
    FunSpec({
        ...
        suspend fun withModuleState(
            ...
            val modesetsOriginal =
                modesetsSection.getKeys(false).associateWith {
        ...
        fun runPacketThroughPacketEvents(
            ...
            val packetType =
                PacketType.getById(PacketSide.SERVER, ConnectionState.PLAY, user.clientVersion, packetId)
        ...
        test("blocked attack sounds are cancelled when module enabled") {
            withModuleState("disable-attack-sounds", enabled = true) {
                ...
                try {
                    val module =
                        ModuleLoader.getModules().filterIsInstance<ModuleAttackSounds>().firstOrNull()
