...
class PlayerRegenIntegrationTest :
    FunSpec({
        ...
        val module =
            ModuleLoader.getModules().filterIsInstance<ModulePlayerRegen>().firstOrNull()
                ...
                runCatching {
                    ...
                    val f =
                        names
                            .asSequence()
                            .mapNotNull { name ->
                                runCatching { ModulePlayerRegen::class.java.getDeclaredField(name) }.getOrNull()
                            }.firstOrNull()
                    if (f != null) {
                        f.isAccessible = true
                        @Suppress("UNCHECKED_CAST")
        ...
        test("non-SATIATED regain reasons are not modified") {
            val nonSatiated =
                EntityRegainHealthEvent.RegainReason.values().firstOrNull {
