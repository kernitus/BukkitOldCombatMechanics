...
class PlayerModuleOverrideApiIntegrationTest :
    FunSpec({
        ...
        test("bulk override setter rejects invalid keys before mutating overrides") {
            runSync {
                ...
                api.getModuleOverridesForPlayer(player) shouldBe
                    mapOf(moduleName to PlayerModuleOverride.FORCE_ENABLED)
                api.getModuleOverrideForPlayer(player, alwaysEnabledModuleName) shouldBe
                ...
                try {
                    shouldThrow<IllegalArgumentException> {
                        api.setModuleOverridesForPlayer(
                            ...
                            mapOf(
                                moduleName to PlayerModuleOverride.FORCE_DISABLED,
                                " ${moduleName.replaceFirstChar { it.uppercase() }} " to
                                    PlayerModuleOverride.DEFAULT,
                                alwaysEnabledModuleName to PlayerModuleOverride.FORCE_DISABLED,
                            ),
                        )
        ...
        test("module override API emits only changed override events in sorted bulk order") {
            runSync {
                ...
                try {
                    ...
                    events.map { it.moduleName } shouldBe
                        listOf(alwaysEnabledModuleName, configDisabledModuleName).sorted()
                    events.map { it.newOverride } shouldBe
                        listOf(
        ...
        test("clearing overrides returns to configured and modeset behaviour") {
            runSync {
                ...
                api.getModuleOverrideForPlayer(player, configDisabledModuleName) shouldBe
                    PlayerModuleOverride.DEFAULT
                module.isEnabled(player) shouldBe false
...
private fun callJavaNullBulkOverrideValue(
    api: OldCombatMechanicsAPI,
    player: Player,
    moduleName: String,
) {
