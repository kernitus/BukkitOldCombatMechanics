...
class FishingRodVelocityIntegrationTest :
    FunSpec({
        ...
        fun createFishEvent(
            ...
            for (ctor in ctors) {
                ...
                for (i in paramTypes.indices) {
                    ...
                    args[i] =
                        when {
                            Player::class.java.isAssignableFrom(t) -> {
                                player
                            }
                            ...
                            PlayerFishEvent.State::class.java.isAssignableFrom(t) -> {
                                state
                            }
                            ...
                            t == Boolean::class.javaPrimitiveType -> {
                                false
                            }
                            ...
                            ItemStack::class.java.isAssignableFrom(t) -> {
            ...
            val handler =
                InvocationHandler { _, method, args ->
                    when (method.name) {
                        ...
                        "getWorld" -> {
                            return@InvocationHandler player.world
                        }

                        "getLocation" -> {
                            return@InvocationHandler player.location.clone()
                        }
                    }
        ...
        test("sets hook velocity to the 1.8 formula (deterministic random seed)") {
            ...
            try {
                runSync {
                    // Keep everything in a single main-thread slice so hook physics cannot
                    // tick between steps (legacy servers are sensitive).
        ...
        test("does not modify hook velocity for non-FISHING states") {
            ...
            try {
                val nonFishing =
                    PlayerFishEvent.State.values().firstOrNull { it != PlayerFishEvent.State.FISHING }
